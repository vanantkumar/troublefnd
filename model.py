...
class FakeNewsClassifier:
    ...
    # ── Public API     def predict(self, text: str) -> dict:
        """
...
def api_predict(text: str) -> dict:
    """
    Calls Gemini API (or any OpenAI-compatible endpoint) for prediction.
    Returns same dict structure as predict() but verdict only — no explanation.
    """
    try:
        ...
        prompt = (
            "You are a fake news classifier. Analyze the following news text. "
            "Reply with ONLY one of these three words, nothing else: "
            "LIKELY_REAL or LIKELY_FAKE or UNCERTAIN\n\n"
            f"Text: {text[:2000]}"
        )
        response = model.generate_content(prompt)
        raw = response.text.strip().upper().replace(" ", "_")
        if "FAKE" in raw:
            verdict = "LIKELY_FAKE"
        elif "REAL" in raw:
            verdict = "LIKELY_REAL"
        ...
        real_prob = 80 if verdict == "LIKELY_REAL" else (20 if verdict == "LIKELY_FAKE" else 50)
        return {
            "verdict": verdict,
            "real_prob": real_prob,
            "fake_score": 0.0,
        ...
        return predict(text)   # silent fallback to ML
